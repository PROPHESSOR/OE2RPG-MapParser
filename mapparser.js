...
class Parser {
    ...
    parse() {
        ...
        const blacklistFilename = `${this.from.slice(0, -3)}blacklist.json`;
        let blacklist = [];

        if (fs.existsSync(blacklistFilename)) {
            console.log(`Using ${blacklistFilename}`);
            blacklist = require('./' + blacklistFilename);
        }

        fs.writeFileSync(this.to, this.generate(map.lines, texmap, map.things, map.decals, map.bspheader, blacklist));
    ...
    generate(lines, texmap, things, decals, bspheader, blacklist) {
